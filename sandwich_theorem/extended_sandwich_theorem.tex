...
{\Large
\begin{center}
\bf{\LARGE \bfseries A Variant of the Sandwich theorem for limits}
\end{center}}
\begin{center}
...
\(\displaystyle \lim_{x \to 0^-} \frac{|x|}{x}=-1\) \hspace{5mm} and \hspace{5cm} \(\displaystyle \lim_{x \to 0^+} \frac{|x|}{x}=1\).

\vspace{5mm}
\section{The Thakuri's Variant of Sandwich Theorem}
\begin{theorem}[The Thakuri's Extended Sandwich Theorem]
Suppose $g(x) \leq f(x) \leq h(x)$ in some open interval containing $c$, expect possibly at $x=c$ itself. Suppose also that $$\lim_{x \to c^-} g(x) = \lim_{x \to c^+} h(x) =L  .$$ Then $\displaystyle \lim_{x \to c} f(x)=L$.
\end{theorem}

Now using this Extended Sandwich theorem it can be conclude that \(\displaystyle \lim_{x \to 0} \frac{sinx}{x}=1\) as follows. \\[2mm]
...
\vspace{5mm}
 \section{Proof of the Extended Sandwich Theorem}
